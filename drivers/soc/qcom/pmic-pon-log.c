...
typedef struct POWERON_TELEMETRY
{
	...
	uint8_t PmicSdamBlock[MAX_PMIC_SDAM_BLOCK_LEN];
} *PPOWERON_TELEMETRY;
...
static int init_telemetry_pmic_sdam_block()
{
	...
	ret = get_pmic_sdam_block(sdam_block_value);
	if (ret < 0) {
		goto init_pmic_sdam_block_exit;
	}
	memcpy(PowerOnTelemetry.PmicSdamBlock, sdam_block_value,
				sizeof(PowerOnTelemetry.PmicSdamBlock));
...
static ssize_t telem_poweron_reset_reason_read(struct kobject *kobj, struct kobj_attribute *attr,
		      char *buf)
{
	pr_err("PowerOnTelemetry.ResetReason is %s", PowerOnTelemetry.ResetReason);
...
static ssize_t telem_poweron_sdam_block_read(struct kobject *kobj, struct kobj_attribute *attr,
		      char *buf)
{
	memcpy(buf, (char*)&PowerOnTelemetry.PmicSdamBlock, sizeof(PowerOnTelemetry.PmicSdamBlock));
	return sizeof(PowerOnTelemetry.PmicSdamBlock);
}
...
static struct kobj_attribute telem_poweron_sdam_block_attribute =
	__ATTR(sdam_block, 0664, telem_poweron_sdam_block_read, NULL);
...
static struct attribute *attrs[] = {
	...
	&telem_poweron_sdam_block_attribute.attr,
