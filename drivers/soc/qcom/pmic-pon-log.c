...
typedef struct FAULT_BLOCK
{
	...
	char pmic_pon_sid0_pmic_fault_reason1[PMIC_FAULT_BUF_SIZE];
	char pmic_pon_sid0_pmic_fault_reason2[PMIC_FAULT_BUF_SIZE];
	char pmic_pon_sid1_pmic_fault_reason1[PMIC_FAULT_BUF_SIZE];
	char pmic_pon_sid1_pmic_fault_reason2[PMIC_FAULT_BUF_SIZE];
	char pmic_pon_sid2_pmic_fault_reason1[PMIC_FAULT_BUF_SIZE];
	char pmic_pon_sid2_pmic_fault_reason2[PMIC_FAULT_BUF_SIZE];	
} *PFAULT_BLOCK;
...
static int get_pmic_fault_reasons(int sid, u8 *fault_reason1, u8 *fault_reason2, u8 *fault_reason3)
{
#define PMIC_PON_REGISTER_START_ADDR 	0x885
#define PON_PBS_FAULT_REASON1			0x8c7
#define PON_PBS_FAULT_REASON2			0x8c8
#define PON_PBS_FAULT_REASON3			0x8c9

	int fault_reason_index = 0;
	uint8_t pon_block_value[MAX_PMIC_BLOCK_LEN];
	int ret = 0;

	if(sid<0 || sid >= MAX_PMIC_SIZE)
		return -1;

	memset(pon_block_value, 0, sizeof(pon_block_value));
	ret = get_pmic_pon_block(sid, pon_block_value);
	if (ret < 0)
		return -1;

	fault_reason_index = PON_PBS_FAULT_REASON1-PMIC_PON_REGISTER_START_ADDR;
	*fault_reason1 = pon_block_value[fault_reason_index];

	fault_reason_index = PON_PBS_FAULT_REASON2-PMIC_PON_REGISTER_START_ADDR;
	*fault_reason2 = pon_block_value[fault_reason_index];

	fault_reason_index = PON_PBS_FAULT_REASON3-PMIC_PON_REGISTER_START_ADDR;
	*fault_reason3 = pon_block_value[fault_reason_index];

	return 0;

}
static int get_fault_reason_str(u8 fault_reason, char *reasonbuf, int buf_size, const char * const *reasonstr,u8 mask)
{
	int i;
	int pos = 0;
	bool first = true;

	if(buf_size < PMIC_FAULT_BUF_SIZE)
		return -1;

	memset(reasonbuf, 0, buf_size);
	for (i = 0; i < 8; i++) {
		if (fault_reason & BIT(i) & mask) {
			pos += scnprintf(reasonbuf + pos, buf_size - pos,
					"%s%s",	(first ? "" : ", "), reasonstr[i]);
			first = false;
		}
	}
	if(first)
		strcpy(reasonbuf, "None");

	return 0;

}
static int init_telemetry_fault_block_pmic_fault_reasons(void)
{
#define PMIC_SID0		0	
#define PMIC_SID1		1
#define PMIC_SID2		2
	...
	// PMIC_SID0
	if(get_pmic_fault_reasons(PMIC_SID0,&fault_reason1,&fault_reason2,&fault_reason3)){
		return -1;
	}
	pr_info("%s: PMIC_SID0: fault_reason1: 0x%x, fault_reason2: 0x%x, fault_reason3: 0x%x \n", __func__, fault_reason1,fault_reason2,fault_reason3);
	get_fault_reason_str(fault_reason1,PowerOnTelemetry.FaultBlock.pmic_pon_sid0_pmic_fault_reason1,PMIC_FAULT_BUF_SIZE,pmic_pon_fault_reason1,0xff);
	get_fault_reason_str(fault_reason2,PowerOnTelemetry.FaultBlock.pmic_pon_sid0_pmic_fault_reason2,PMIC_FAULT_BUF_SIZE,pmic_pon_fault_reason2,0xbf); // skip RESTART_PON
	
	// PMIC_SID1
	if(get_pmic_fault_reasons(PMIC_SID1,&fault_reason1,&fault_reason2,&fault_reason3)){
		return -1;
	}
	pr_info("%s: PMIC_SID1: fault_reason1: 0x%x, fault_reason2: 0x%x, fault_reason3: 0x%x \n", __func__, fault_reason1,fault_reason2,fault_reason3);
	get_fault_reason_str(fault_reason1,PowerOnTelemetry.FaultBlock.pmic_pon_sid1_pmic_fault_reason1,PMIC_FAULT_BUF_SIZE,pmic_pon_fault_reason1,0xff);
	get_fault_reason_str(fault_reason2,PowerOnTelemetry.FaultBlock.pmic_pon_sid1_pmic_fault_reason2,PMIC_FAULT_BUF_SIZE,pmic_pon_fault_reason2,0xbf); // skip RESTART_PON

	// PMIC_SID2
	if(get_pmic_fault_reasons(PMIC_SID2,&fault_reason1,&fault_reason2,&fault_reason3)){
		return -1;
	}
	pr_info("%s: PMIC_SID1: fault_reason1: 0x%x, fault_reason2: 0x%x, fault_reason3: 0x%x \n", __func__, fault_reason1,fault_reason2,fault_reason3);
	get_fault_reason_str(fault_reason1,PowerOnTelemetry.FaultBlock.pmic_pon_sid2_pmic_fault_reason1,PMIC_FAULT_BUF_SIZE,pmic_pon_fault_reason1,0xff);
	get_fault_reason_str(fault_reason2,PowerOnTelemetry.FaultBlock.pmic_pon_sid2_pmic_fault_reason2,PMIC_FAULT_BUF_SIZE,pmic_pon_fault_reason2,0xbf); // skip RESTART_PON
...
pmic0_fr1=%s\npmic0_fr2=%s\npmic1_fr1=%s\npmic1_fr2=%s\npmic2_fr1=%s\npmic2_fr2=%s\n",
						...
						PowerOnTelemetry.FaultBlock.pmic_pon_sid0_pmic_fault_reason1,
						PowerOnTelemetry.FaultBlock.pmic_pon_sid0_pmic_fault_reason2,
						PowerOnTelemetry.FaultBlock.pmic_pon_sid1_pmic_fault_reason1,
						PowerOnTelemetry.FaultBlock.pmic_pon_sid1_pmic_fault_reason2,
						PowerOnTelemetry.FaultBlock.pmic_pon_sid2_pmic_fault_reason1,
						PowerOnTelemetry.FaultBlock.pmic_pon_sid2_pmic_fault_reason2);
}
...
static int pmic_pon_log_probe(struct platform_device *pdev)
{
	...
	//Set string to sid0, sid1 and sid2 fault reasons of PowerOnTelemetry.FaultBlock
	ret = init_telemetry_fault_block_pmic_fault_reasons();
	if (ret < 0)
		dev_err(&pdev->dev, "PMIC set PMIC fault reason str failed, ret=%d\n",
			ret);
