...
static int spi_hid_power_reset(struct spi_hid *shid)
{
	return pinctrl_select_state(shid->pinctrl, shid->pinctrl_reset);
}

static int spi_hid_power_restart(struct spi_hid *shid)
{
	/* Let VREG_TS_5V0 stabilize */
	usleep_range(10000, 11000);

	return pinctrl_select_state(shid->pinctrl, shid->pinctrl_active);
}
...
static int spi_hid_error_handler(struct spi_hid *shid)
{
	...
	shid->report_descriptor_crc32 = 0;
	...
	ret = spi_hid_power_restart(shid);
	if (ret) {
...
static void spi_hid_create_device_work(struct work_struct *work)
{
	...
	if (shid->desc.hid_version != SPI_HID_SUPPORTED_VERSION) {
		...
		ret = spi_hid_error_handler(shid);
		if (ret)
			dev_err(dev, "Error handler failed\n");
		return;
	}
