...
static int hw_reset(struct fpc1020_data *fpc1020)
{
	...
	irq_gpio = gpio_get_value(fpc1020->irq_gpio);
	// return nonzero if failed to trigger IRQ probably due to
	// the broken reset connection between MCU and sensor
	if (irq_gpio == 0) rc = 1;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(!strncmp(buf, "enable", strlen("enable")) && fpc1020->compatible_enabled != 1){
		...
		if (of_property_read_bool(dev->of_node, "fpc,enable-wakeup")) {
		    // Enabling no suspend flag and registering interrupt as wake-capable is not
		    // allowed by power manager and leads to wake from standby failures.
