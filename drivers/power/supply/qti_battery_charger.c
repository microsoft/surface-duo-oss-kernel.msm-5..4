...
enum ms_property_id {
	...
	MS_SEC_CHG_ICL_MA,
	MS_SEC_CHG_RESET,
	MS_SEC_CHG_SHIP_MODE,
	MS_SEC_CHG_VBAT,
	MS_SEC_CHG_OVERRIDE_ICL_SPLIT_PERCENTAGE,
	MS_SEC_CHG_OVERRIDE_FCC_SPLIT_PERCENTAGE,
	MS_SEC_CHG_MEASURED_ICHGR,
	MS_SEC_CHG_REG_READ,
	MS_SEC_CHG_REG_WRITE,
	MS_PROP_MAX,
};
...
struct battery_chg_dev {
	...
	struct kobject USBTelemetryKobj;
	...
	case MS_GET_REQ:
		if ((len == sizeof(*ms_resp_msg)) && (ms_resp_msg->ret_code == 0) &&
		    ms_resp_msg->property_id < MS_PROP_MAX){
				if(ms_resp_msg->property_id == MS_TELEMETRY || ms_resp_msg->property_id == MS_USB_TELEMETRY){
...
static int sec_chg_icl_read(void *data, u64 *val)
{
	int rc;
	struct ms_propval propval = {
									.property_id = MS_SEC_CHG_ICL_MA,
							};
	rc = get_ms_prop_adsp(data, &propval);
	*val = propval.intval;
	return rc;
}
DEFINE_SIMPLE_ATTRIBUTE(sec_chg_icl_ops, sec_chg_icl_read,
			NULL, "%d\n");

static int sec_chg_reset(void *data, u64 val)
{
	struct ms_propval propval = {
									.property_id = MS_SEC_CHG_RESET,
									.intval 	 = val,
							};
	return set_ms_prop_adsp(data, propval, NULL);
}
DEFINE_SIMPLE_ATTRIBUTE(sec_chg_reset_ops, NULL,
			sec_chg_reset, "%d\n");

static int sec_chg_ship_mode(void *data, u64 val)
{
	struct ms_propval propval = {
									.property_id = MS_SEC_CHG_SHIP_MODE,
									.intval 	 = val,
							};
	return set_ms_prop_adsp(data, propval, NULL);
}
DEFINE_SIMPLE_ATTRIBUTE(sec_chg_ship_mode_ops, NULL,
			sec_chg_ship_mode, "%d\n");

static int sec_chg_vbat_read(void *data, u64 *val)
{
	int rc;
	struct ms_propval propval = {
									.property_id = MS_SEC_CHG_VBAT,
							};
	rc = get_ms_prop_adsp(data, &propval);
	*val = propval.intval;
	return rc;
}
DEFINE_SIMPLE_ATTRIBUTE(sec_chg_vbat_ops, sec_chg_vbat_read,
			NULL, "%d\n");

static int sec_chg_get_override_icl_split_percentage(void *data, u64 *val)
{
	int rc;
	struct ms_propval propval = {
									.property_id = MS_SEC_CHG_OVERRIDE_ICL_SPLIT_PERCENTAGE,
							};
	rc = get_ms_prop_adsp(data, &propval);
	*val = propval.intval;
	return rc;
}
static int sec_chg_set_override_icl_split_percentage(void *data, u64 val)
{
	struct ms_propval propval = {
									.property_id = 	MS_SEC_CHG_OVERRIDE_ICL_SPLIT_PERCENTAGE,
									.intval 	 = val,
...
DEFINE_SIMPLE_ATTRIBUTE(sec_chg_set_icl_split_per_ops, sec_chg_get_override_icl_split_percentage,
			sec_chg_set_override_icl_split_percentage, "%d\n");

static int sec_chg_get_override_fcc_split_percentage(void *data, u64 *val)
{
	int rc;
	struct ms_propval propval = {
									.property_id = MS_SEC_CHG_OVERRIDE_FCC_SPLIT_PERCENTAGE,
							};
	rc = get_ms_prop_adsp(data, &propval);
	*val = propval.intval;
	return rc;
}
static int sec_chg_set_override_fcc_split_percentage(void *data, u64 val)
{
	struct ms_propval propval = {
									.property_id = 	MS_SEC_CHG_OVERRIDE_FCC_SPLIT_PERCENTAGE,
									.intval 	 = val,
...
DEFINE_SIMPLE_ATTRIBUTE(sec_chg_set_fcc_split_per_ops, sec_chg_get_override_fcc_split_percentage,
			sec_chg_set_override_fcc_split_percentage, "%d\n");

static int sec_chg_get_measured_ichgr(void *data, u64 *val)
{
	int rc;
	struct ms_propval propval = {
									.property_id = MS_SEC_CHG_MEASURED_ICHGR,
							};
	rc = get_ms_prop_adsp(data, &propval);
	*val = propval.intval;
	return rc;
}
DEFINE_SIMPLE_ATTRIBUTE(sec_chg_get_measured_ichgr_ops, sec_chg_get_measured_ichgr,
			NULL, "%d\n");

#ifdef CONFIG_DEBUG_FS

static int sec_chg_reg_read(void *data, unsigned int reg, unsigned int *val)
{
	int rc = 0;
	struct ms_propval propval = {
									.property_id = 	MS_SEC_CHG_REG_READ,
									.intval 	 = reg,
								};
	rc = get_ms_prop_adsp(data, &propval);
	*val = propval.intval;
...
static int sec_chg_reg_write(void *data, unsigned int reg, unsigned int val)
{
	struct ms_propval propval = {
									.property_id = 	MS_SEC_CHG_REG_WRITE,
									.intval 	 = 0,
							};
	propval.intval = reg << 8 | val;
	return set_ms_prop_adsp(data, propval, NULL);

}
static const struct regmap_config sec_chg_regmap_cfg = {
	.name		= "sec_chg",
	.reg_bits	= 8,
	.reg_stride	= 1,
	.val_bits	= 8,
	.reg_read	= sec_chg_reg_read,
	.reg_write	= sec_chg_reg_write,
	.max_register	= 21,
};

static int battery_chg_create_sec_chg_debugfs(struct battery_chg_dev *bcdev,
				       struct dentry *chg_root_dir)
{
	int rc;
	struct dentry *sec_chg_root_dir;
	struct regmap *regmap = NULL;
	char regdump_link_name[100];

	sec_chg_root_dir = debugfs_create_dir("sec_chg", chg_root_dir);
	if (IS_ERR(sec_chg_root_dir)) {
		pr_err("Failed to create secondary charger root directory\n");
		return PTR_ERR(sec_chg_root_dir);
	}

	rc = PTR_ERR_OR_ZERO(debugfs_create_file("icl", 0400, sec_chg_root_dir,
			bcdev, &sec_chg_icl_ops));
	if (rc)
		goto error;

	rc = PTR_ERR_OR_ZERO(debugfs_create_file("reset_secondary_charger", 0200, sec_chg_root_dir,
			bcdev, &sec_chg_reset_ops));
	if (rc){
		pr_err("Failed to create reset_secondary_charger file");
		goto error;
	}

	rc = PTR_ERR_OR_ZERO(debugfs_create_file("ship_mode", 0200, sec_chg_root_dir,
			bcdev, &sec_chg_ship_mode_ops));
	if (rc){
		pr_err("Failed to create ship_mode file");
		goto error;
	}

	rc = PTR_ERR_OR_ZERO(debugfs_create_file("vbat", 0400, sec_chg_root_dir,
			bcdev, &sec_chg_vbat_ops));
	if (rc){
		pr_err("Failed to create vbat file");
		goto error;
	}

	rc = PTR_ERR_OR_ZERO(debugfs_create_file("override_icl_split_per", 0600, sec_chg_root_dir,
			bcdev, &sec_chg_set_icl_split_per_ops));
	if (rc){
		pr_err("Failed to create override_icl_split_per file");
		goto error;
	}

	rc = PTR_ERR_OR_ZERO(debugfs_create_file("override_fcc_split_per", 0600, sec_chg_root_dir,
			bcdev, &sec_chg_set_fcc_split_per_ops));
	if (rc){
		pr_err("Failed to create override_fcc_split_per file");
		goto error;
	}

	rc = PTR_ERR_OR_ZERO(debugfs_create_file("measured_ibat", 0400, sec_chg_root_dir,
			bcdev, &sec_chg_get_measured_ichgr_ops));
	if (rc){
		pr_err("Failed to create measured_ibat file");
		goto error;
	}
	regmap = devm_regmap_init(bcdev->dev, NULL, bcdev, &sec_chg_regmap_cfg);
	if (IS_ERR(regmap)) {
		pr_err("Failed to init regmap for secondary charger\n");
		goto error;
	}
	snprintf(regdump_link_name, sizeof(regdump_link_name), "../../regmap/%s-%s",dev_name(bcdev->dev),sec_chg_regmap_cfg.name);

	rc = PTR_ERR_OR_ZERO(debugfs_create_symlink("regmap", sec_chg_root_dir, regdump_link_name));

	if (rc) {
		pr_err("Failed to crete sym link node for secondary charger regmap\n");
	}

	return 0;

	error:
		pr_err("battery_chg_create_sec_chg_debugfs Failed to create debugfs file: %d\n", rc);
		return rc;
}
...
static void battery_chg_add_debugfs(struct battery_chg_dev *bcdev)
{
	...
	rc = battery_chg_create_sec_chg_debugfs(bcdev, dir);
	if (rc) {
		pr_err("Failed to create seccondary charger dir and files: %d\n", rc);
		goto error;
	}
...
static ssize_t ms_usb_telemetry_payload_read(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int rc;
	struct ms_propval propval = {
									.property_id = MS_USB_TELEMETRY,
							};
	struct battery_chg_dev *bcdev = container_of(kobj, struct battery_chg_dev,
						     USBTelemetryKobj);
	mutex_lock(&bcdev->BatteryTelemetryLock);
	...
	if(rc)
	{
		pr_err("usb telemetry payload read failed rc :%d", rc);
		mutex_unlock(&bcdev->BatteryTelemetryLock);
		return -EFAULT;
	}
	memcpy(buf,&bcdev->ms_buf,propval.intval);
...
static struct kobj_attribute ms_usb_telemetry_payload_attribute =
	__ATTR(usb_telemetry_payload, 0664, ms_usb_telemetry_payload_read, NULL);

static struct attribute *attrs_usb_telemetry[] = {
    &ms_usb_telemetry_payload_attribute.attr,
    NULL,	/* need to NULL terminate the list of attributes */
};
static struct attribute_group attr_group_usb_telemetry = {
	.attrs = attrs_usb_telemetry,
};
static const struct sysfs_ops telem_usb_sysfs_ops = {
	.show	= telem_sysfs_show,
};
static struct kobj_type telem_usb_kobj_ktype = {
	.sysfs_ops	= &telem_usb_sysfs_ops,
};

int ms_usb_telemetry_fs(struct battery_chg_dev *bcdev){

	int rc;
	rc = telemetry_init();
	if (rc < 0)
	{
		pr_err("USBTelemetryInit: failed to create telemetry parent obj");
		goto Exit;
	}

    // Setup sysfs directory for usb telemetry
	 rc = kobject_init_and_add(&bcdev->USBTelemetryKobj, &telem_usb_kobj_ktype, telemetry_kobj, "usb");

    if (rc)
    {
        pr_err("USBTelemetryInit: failed to create usb parent obj");
        rc = -ENOMEM;
        kobject_put(&bcdev->USBTelemetryKobj); //decrease reference count
        goto Exit;
    }

    // Add 'files' under the sysfs directory for usb telemetry
    rc = sysfs_create_group(&bcdev->USBTelemetryKobj, &attr_group_usb_telemetry);
    if (rc)
    {
        pr_err("USBTelemetryInit: failed to add sysfs group for usb telemetry");
        rc = -EINVAL;
        kobject_del(&bcdev->USBTelemetryKobj);
        goto Exit;
    }
Exit:
    return rc;
