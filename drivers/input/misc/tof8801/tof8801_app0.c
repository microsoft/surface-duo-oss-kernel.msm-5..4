...
int tof8801_app0_is_v2(void *tof_chip) {
    ...
    // Check for version >= v2
    // (assume v3+ is backward compatible with v2)
...
int tof8801_app0_wait_for_idle(void *tof_chip, unsigned long usec_timeout)
{
    ...
    struct tof_sensor_chip *chip = (struct tof_sensor_chip *)tof_chip;
    do {
        error = tof8801_get_register(chip, TOF8801_STAT, &cpu_stat);
        if (error == 0) {
            error = tof8801_get_register(chip, OL_STATE_OFFSET, &state);
    ...
    dev_err(chip->dev,
            "Error App0 timeout (%lu us) waiting on idle; cpu_stat: %#x state: %#x\n",
...
int tof8801_app0_rw_osc_trim(void *tof_chip, int *trim_parm, int write_op) {
    ...
    if (!trim_parm) {
        dev_err(chip->dev, "Error: trim value is null pointer");
        return -1;
    }
    ...
    if (error) {
        dev_err(chip->dev, "Error (%d): stopping capture", error);
        return error;
    }

    error = tof8801_set_register(chip, OL_CMD_DATA9_OFFSET,
            TOF8801_APP0_OSC_TRIM_CMD);
    if (error) {
        dev_err(chip->dev, "Error (%d): writing osc_trim command", error);
        return error;
    }
    error = tof8801_set_register(chip, TOF8801_STAT, 0x00);
    if (error) {
        dev_err(chip->dev, "Error (%d): setting PON0", error);
        return error;
    }
    usleep_range(5000, 5500);

    error = tof8801_get_register(chip, 0x03, &fuse3);
    if (error) {
        dev_err(chip->dev, "Error (%d): reading Fuse_3", error);
        return error;
    }
    error = tof8801_get_register(chip, 0x06, &fuse6);
    if (error) {
        dev_err(chip->dev, "Error (%d): reading Fuse_6", error);
        return error;
    }
    ...
    if (write_op) {
        trim_val = *trim_parm;
        trim_val &= 0x1FF;
        dev_info(chip->dev, "%s: new osc_trim: %#x", __func__, trim_val);
        fuse6 &= ~(0x01 << 6);
        fuse6 |= ((trim_val & 0x01) << 6);
        fuse3  = ((trim_val >> 1) & 0xFF);
        error = tof8801_set_register(chip, 0x03, (char)fuse3);
        if (error) {
            dev_err(chip->dev, "Error (%d): writing Fuse_3", error);
            return error;
        }
        error = tof8801_set_register(chip, 0x06, (char)fuse6);
        if (error) {
            dev_err(chip->dev, "Error (%d): writing Fuse_6", error);
            return error;
        }
    } else {
        trim_val = 0;
        ...
        *trim_parm = trim_val;
        dev_info(chip->dev, "%s: current osc_trim: %#x, clk_ratio "
                "(Q15): %u", __func__, trim_val,
                chip->app0_app.clk_cr.ratioQ15);
    }

    error = tof8801_set_register(chip, TOF8801_STAT, 0x01);
    if (error) {
        dev_err(chip->dev, "Error (%d): setting PON1", error);
        return error;
    }
    ...
    if (error) {
        dev_err(chip->dev, "Error (%d): idle state timeout", error);
        return error;
    }
    ...
    if (save_state) {
        ...
        if (error) {
            dev_err(chip->dev, "Error (%d): trying to restart capture", error);
            return error;
        }
    }
...
static int tof8801_app0_osc_trim(struct tof_sensor_chip *chip, int change)
{
    ...
    error = tof8801_app0_capture(chip, 0);
    if (error) {
        dev_err(chip->dev, "Error (%d): stopping capture",
                error);
        return error;
    }
        ...
        unsigned int ratio, int qf)
{
    ...
    if (period >= 6 && is_prox_only) {
        ...
        if (chip->driver_debug) {
            dev_info(chip->dev, "PLL_correction (Q%d): "
                    "%u -> %u\n", qf, ratio, new_ratio);
        }
    }
    return new_ratio;
...
static int tof8801_app0_check_osc(struct tof_sensor_chip *chip)
{
    ...
    if (chip->app0_app.clk_cr.trim_count >= chip->app0_app.clk_iterations) {
        ...
        if (chip->app0_app.clk_trim_enable) {
            ...
            if (trim == 0 && stable != 1) {
                dev_info(chip->dev, "%s: clk_ratio (Q15): %u "
                        "is within tolerance\n",
...
static int app0_wait_for_tid_change(struct tof_sensor_chip *chip,
    ...
    do {
        tries++;
        error = tof8801_get_register(chip, OL_TID_OFFSET, &new_tid);
        if (error) {
    ...
    if (tries >= num_tries) {
        dev_err(chip->dev, "Error waiting on TID");
...
static int tof8801_app0_read_data_set_cmd(struct tof_sensor_chip *chip,
    ...
    error = tof8801_set_register(chip, OL_COMMAND_OFFSET, cmd);
    if (error) {
        dev_err(chip->dev, "Error writing cmd \'%x\': %d", cmd, error);
        return error;
    }
    error = app0_wait_for_tid_change(chip, TOF8801_MAX_WAIT_RETRY);
    if (error) {
        dev_err(chip->dev, "Error waiting on TID for cmd \'%x\': %d", cmd, error);
        return error;
    }
    //read buffer
    error = tof8801_get_register_list(chip, TOF8801_APP0_DATA_START, wbuf, size);
    if (error) {
        dev_err(chip->dev, "Error reading data set buffer: %d", error);
        return error;
    }
    return error;
}
...
int tof8801_app0_perform_factory_calibration(void *tof_chip)
{
    ...
    error = tof8801_set_register(chip, OL_COMMAND_OFFSET, fac_calib_cmd);
...
int tof8801_app0_write_calibration(void *tof_chip)
{
    ...
    dev_info(chip->dev, "Start calibration upload");

    if (update->dataFactoryConfig && (chip->ext_calib_data.size == 0)) {
        dev_info(chip->dev, "Calibration data not present, ignore");
        return 0;
    }
    ...
    data_0->dataAlgorithmState = (data_0->dataFactoryConfig &&
                                  update->dataAlgorithmState &&
                                  (chip->alg_info.size != 0));
    data_0->dataConfiguration = (update->dataConfiguration &&
                                 (chip->config_data.size != 0));
    calib_len = 0;
    if (data_0->dataFactoryConfig) {
        dev_info(chip->dev, "Factory Calibration data added");
    ...
    if (data_0->dataAlgorithmState) {
        dev_info(chip->dev, "Algorithm state data added");
    ...
    if (data_0->dataConfiguration) {
        dev_info(chip->dev, "Configuration data added");
        memcpy(calib_buf + calib_len,
    ...
    if (calib_len == 0) {
        dev_info(chip->dev, "No calibration data to upload.");
    } else {
        error = tof8801_set_register_list(chip, OL_FACTORY_CALIB_OFFSET,
                calib_buf, calib_len);
        if (error) {
            dev_err(chip->dev, "Error writing calibration data: %d", error);
            return 0;
        }
        dev_info(chip->dev, "Calibration data uploaded.");
...
int tof8801_app0_get_version(void * tof_chip, char *str, unsigned int strlen)
{
    ...
    if (chip->driver_debug) {
        dev_info(chip->dev, "App0 ver_str: \'%s\'", str);
    }
    return len;
}
...
int tof8801_app0_read_version(struct tof_sensor_chip *chip) {
    ...
    error = tof8801_get_register(chip,
    ...
    dev_info(chip->dev, "App0 version: %d-%d-%d-%d",
            chip->app0_app.version.maj_rev,
            chip->app0_app.version.min_rev,
            chip->app0_app.version.patch_rev,
            BYTES_TO_16B(chip->app0_app.version.build_0_rev,
...
void tof8801_app0_init_app(struct tof8801_app0_application *app0) {
    ...
    // Need to appropriately change the clock iteration counter
...
int tof8801_app0_capture(void *tof_chip, int capture)
{
    ...
    if (capture) {
        ...
        calib_len = tof8801_app0_write_calibration(chip);
        chip->app0_app.cap_settings.cmd =
            tof8801_app0_is_v2(chip) ? OL_COMMAND_START_EXT_CAL : OL_COMMAND_START;
        if ((chip->app0_app.cap_settings.cmd == OL_COMMAND_START_EXT_CAL) &&
            (calib_len == 0)) {
            chip->app0_app.cap_settings.cmd = OL_COMMAND_START_EXT;
        }
        error = tof8801_set_register_list(chip, TOF8801_APP0_CMD_START,
        ...
        //stop measurement
        error = tof8801_set_register(chip, OL_COMMAND_OFFSET, OL_COMMAND_STOP);
        if (error) {
            MEASURE_UNLOCK(chip->app0_app.measure_in_prog);
            dev_err(chip->dev, "Error sending STOP cmd");
            return error;
        }
        //clear pending interrupts
        error = tof8801_set_register(chip, TOF8801_INT_STAT, 0);
        if (error) {
            MEASURE_UNLOCK(chip->app0_app.measure_in_prog);
            dev_err(chip->dev, "Error clearing INT_STAT");
            return error;
        }
        error = tof8801_app0_wait_for_idle(chip, TOF8801_APP0_IDLE_WAIT_TIMEOUT);
        if (error) {
            dev_err(chip->dev, "Error waiting for IDLE state");
            goto stop_err;
        }
        //Read out APP0 header info: status, last cmd, TID, register contents, etc
        error = tof8801_get_register_list(chip,
                OL_PREVIOUS_OFFSET,
                &chip->app0_app.ctrl_frame.buf[OL_PREVIOUS_OFFSET],
                (sizeof(struct tof8801_app0_control_reg_frame) - OL_PREVIOUS_OFFSET));
        if (error) {
            dev_err(chip->dev, "Error reading ctrl_reg");
            tof8801_app0_report_error(chip, ERR_COMM, DEV_OK);
            goto stop_err;
        }
        // Keep current correction, but let clock correction re-settle
        // next capture period
...
void tof8801_app0_measure_timer_expiry_callback(struct timer_list *timer)
{
    ...
    struct tof_sensor_chip *chip = from_timer(chip, timer, meas_timer);
    MEASURE_UNLOCK(chip->app0_app.measure_in_prog);
...
int tof8801_app0_switch_to_bootloader(void *tof_chip)
{
    int error;
    struct tof_sensor_chip *chip = (struct tof_sensor_chip *)tof_chip;
    error = tof8801_set_register(chip,
            TOF8801_REQ_APP_ID,
    ...
    error = tof_wait_for_cpu_ready_timeout(chip, 10000);
    if (error) {
        return error;
    }
...
int tof8801_app0_get_v1_results(struct tof_sensor_chip *chip)
{
    ...
    int error = 0;
    if (num_results > 0) {
        error = tof8801_get_register_list(chip, OL_RESULT_NUMBER_OFFSET,
            rbuf, results_size);
    }
    return error;
}
...
int tof8801_app0_get_v2_results(struct tof_sensor_chip *chip)
{
    ...
    error = tof8801_get_register_list(chip, OL_STATUS_OFFSET, (char *)result,
...
int tof8801_app0_read_results(struct tof_sensor_chip *chip)
{
    ...
    if (error) {
        dev_err(chip->dev, "IRQ: Error reading result from device");
    ...
    if (error) {
        dev_err(chip->dev, "IRQ: Error adding result frame to fifo");
    }
    return error;
}
        ...
        int size)
{
    ...
    for (i = 0; i < num_chunks; i++) {
        error = tof8801_get_register_list(chip, TOF8801_APP0_DATA_START,
        ...
        tof_core1_diag_info_t diag_info)
{
    ...
    error = tof8801_set_register(chip, OL_COMMAND_OFFSET,
            OL_COMMAND_RD_HISTO);
    if (error) {
        tof8801_app0_report_error(chip, ERR_COMM, DEV_OK);
    }
...
{
    int error = tof8801_get_register_list(chip, OL_FACTORY_CALIB_OFFSET,
    ...
    dev_info(chip->dev, "Read calibration data (%d bytes)",
        chip->ext_calib_data.size);
    return;
}
...
void tof8801_app0_process_irq(void *tof_chip, char int_stat)
{
    ...
    error = tof8801_get_register_list(chip,
            ...
            (sizeof(struct tof8801_app0_control_reg_frame) - OL_PREVIOUS_OFFSET));
    if (error) {
        dev_err(chip->dev, "IRQ: Error reading ctrl_reg");
        tof8801_app0_report_error(chip, ERR_COMM, DEV_OK);
    ...
    if (int_stat & IRQ_RESULTS) {
        if (chip->driver_debug) {
            dev_info(chip->dev, "IRQ: Received RESULTS interrupt");
        }
    ...
    if (int_stat & IRQ_DIAG) {
        ...
        if (chip->driver_debug) {
            dev_info(chip->dev, "IRQ: Received DIAG interrupt");
            dev_info(chip->dev,
        ...
        tof8801_set_register(chip, OL_COMMAND_OFFSET, OL_COMMAND_CONTINUE);
    }
}
        ...
        struct tof_sensor_chip *chip)
{
    ...
    if (error) {
        dev_err(chip->dev, "Error starting measure timer: %d", error);
        return error;
    }
    error = tof8801_set_register_list(chip, TOF8801_APP0_CMD_START,
...
int tof8801_app0_get_dataset(void *tof_chip, int dataset_type)
{
    ...
    switch(dataset_type) {
        case GEN_CFG_DATASET:
            ...
            break;
        default:
            dev_err(chip->dev, "Error unsupported dataset type \'%d\'", dataset_type);
    }
    error = tof8801_get_register(chip,
            OL_TID_OFFSET,
