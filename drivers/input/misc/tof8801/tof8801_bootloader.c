 ...
 */
int is_BL_cmd_busy(struct tof_sensor_chip *chip)
{
    char status = CMD_BUSY;
    tof8801_get_register(chip, TOF8801_CMD_STAT, &status);
 ...
 */
int tof8801_BL_read_status(struct tof_sensor_chip *chip,
        ...
        int num_retries)
{
    ...
    do {
        num_retries -= 1;
        error = tof8801_get_register_list(chip, TOF8801_CMD_STAT,
        ...
        if (TOF8801_BL_IS_CMD_BUSY(*status)) {
            ...
            if (num_retries <= 0) {
                dev_info(chip->dev, "BL application is busy: %#04x", *status);
                error = -EBUSY;
            }
            continue;
        }
    ...
    dev_dbg(chip->dev, "BL application wait for response: \'%d\'", error);
    return error;
}
 ...
 */
int tof8801_BL_send_rcv_cmd(struct tof_sensor_chip *chip,
    struct tof8801_BL_application *BL_app) {
    int error = -1;
    char *wbuf = get_BL_cmd_buf(BL_app);
    char wsize = TOF8801_CALC_BL_CMD_SIZE(wbuf[1]);
    if (is_BL_cmd_busy(chip))
        return error;

    error = tof8801_set_register_list(chip, TOF8801_CMD_STAT, wbuf, wsize);
    if (error)
        return error;

    return tof8801_BL_read_status(chip, BL_app, 5);
}
 ...
 * @BL_app: pointer to BL application struct
 */
int tof8801_BL_send_cmd(struct tof_sensor_chip *chip,
    struct tof8801_BL_application *BL_app) {
    int error = -1;
    char *wbuf = get_BL_cmd_buf(BL_app);
    char wsize = TOF8801_CALC_BL_CMD_SIZE(wbuf[1]);
 ...
 * @BL_app: pointer to BL application struct
 */
int tof8801_BL_short_cmd(struct tof_sensor_chip *chip,
        struct tof8801_BL_application *BL_app,
        enum tof8801_bootloader_cmd cmd_e)
{
    ...
    return tof8801_BL_send_rcv_cmd(chip, BL_app);
}
 ...
 * @BL_app: pointer to BL application struct
 */
int tof8801_BL_reset(struct tof_sensor_chip *chip,
        struct tof8801_BL_application *BL_app)
{
    int error;
    struct tof8801_BL_short_cmd *cmd = &(BL_app->BL_command.short_cmd);
    ...
    error = tof8801_BL_send_cmd(chip, BL_app);
    if (error)
        return error;
    return tof_wait_for_cpu_ready(chip);
}
 ...
 * @BL_app: pointer to BL application struct
 */
int tof8801_BL_ram_remap(struct tof_sensor_chip *chip,
        struct tof8801_BL_application *BL_app)
{
    int error;
    struct tof8801_BL_short_cmd *cmd = &(BL_app->BL_command.short_cmd);
    ...
    if (error)
        return error;
    return tof_wait_for_cpu_startup(chip);
}
 ...
 * @BL_app: pointer to BL application struct
 */
int tof8801_BL_addr_ram(struct tof_sensor_chip *chip,
        struct tof8801_BL_application *BL_app,
        int addr)
{
 ...
 */
int tof8801_BL_read_ram(struct tof_sensor_chip *chip,
        struct tof8801_BL_application *BL_app,
        char * rbuf, int len)
{
    int error = 0;
    ...
    do {
        ...
        cmd->chksum = tof8801_calc_chksum(get_BL_cmd_buf(BL_app),
                TOF8801_CALC_CHKSUM_SIZE(cmd->size));
        rc = tof8801_BL_send_rcv_cmd(chip, BL_app);
        if (!rc) {
 ...
 */
int tof8801_BL_write_ram(struct tof_sensor_chip *chip,
        struct tof8801_BL_application *BL_app,
 ...
 */
int tof8801_BL_upload_init(struct tof_sensor_chip *chip,
        struct tof8801_BL_application *BL_app,
        char salt)
{
