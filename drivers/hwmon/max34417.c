...
struct max34417_data {
	...
	int trigger_to_update ;  // 0 for stopping read and update handler and 1 for starting read and update handler.
	...
	const char *max_label;  // Name of the respective debugfs node to start and stop read/update handler.
};
...
void max34417_read_handler(struct work_struct *work) {
	...
	for (i = 0; i < 4; i++) {
		...
		if( strcmp(data->ch_label[i], "N/A")!=0)
		{
	...
	if(data->trigger_to_update)
    {
        schedule_delayed_work(&data->update_work, msecs_to_jiffies(update_interval - MAX34417_UPDATE_DELAY_MS));
    }
}
...
// debugfs nodes for read and update handler.

static int debugfs_write(void *data, u64 val)
{
	struct max34417_data *temp_data = data;
	temp_data->trigger_to_update = val;
	if(val)
	{
		schedule_delayed_work(&temp_data->update_work, 0);
	}
	return 0;
}

static int debugfs_read(void *data, u64 *val)
{
	struct max34417_data *temp_data = data;
	*val = temp_data->trigger_to_update;
	return 0;
}

DEFINE_SIMPLE_ATTRIBUTE(read_write_ops, debugfs_read, debugfs_write, "%d\n");

void update_control_debugfs(struct max34417_data *data)
{
    int rc;
	struct dentry *dir;
	dir = debugfs_create_dir(data->max_label, NULL);
	if (IS_ERR(dir)) {
		rc = PTR_ERR(dir);
		pr_info("could not create control_update_handler directory, rc=%d\n", rc);
		return;
	}
	debugfs_create_file("control_update_handler", 0666, dir, data, &read_write_ops);
	return;
}
// debugfs_end
				...
				struct max34417_data *data)
{
	...
	if(!of_property_read_string(client->dev.of_node, "max_label", &max_label))
	{
		data->max_label = max_label;
	}
	else
	{
		pr_info("max_label property not found.\n");
	}
				...
				if (reg >= 0 && reg < 4) {
					if (!of_property_read_u32(channel_node, "sense-resistor-milliohm",
						...
						if (!of_property_read_string(channel_node, "label", &label))
							...
							strlcpy(data->ch_label[reg], unknown_label, sizeof(data->ch_label[reg]));
					} else {
						dev_err(&client->dev,
				...
				const struct i2c_device_id *id)
{
	...
	update_control_debugfs(data); // setting up debugfs node for read and update handler.
