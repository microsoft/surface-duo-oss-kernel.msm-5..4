...
typedef struct XBL_HLOS_HOB
{
    ...
    uint8_t DisplayRDID[DISPLAY_RDID_LEN];    // (10) Indicates which TDM RDID has been read.
                                              ...
                                              //      0x9        - Left side
                                              //      0x10       - right side
    uint8_t CrsIdentifier;                    //(11)  Crs Identifier
                                              //0x01       - Crs Identifier bit to diffrentiate Zeta and Crs[Crs->0x0, Zeta->0x1].
                                              //0x10       - Crs power debug identifier [Crs PwrDb board-> 0x10, Crs FF board->0x00, Zeta->0x01]

    uint8_t PmicPonBlock[MAX_PMIC_SIZE][MAX_PMIC_BLOCK_LEN]; //(12) PMIC pon block from 0x885 to 0x8cb
    uint8_t PmicSdamBlock05[MAX_PMIC_SDAM_BLOCK_LEN]; // PMIC sdam block from 0x7440 to 0x74BF
    uint8_t PmicSdamBlock01[MAX_PMIC_SDAM_BLOCK_LEN];  // PMIC SDAM5 block from 0x7040 to 0x70BF
